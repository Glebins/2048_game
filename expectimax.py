...
class ExpectimaxTree:
    def __init__(self, game):
        self.game = game
        self.root = ExpectimaxNode(game.get_state())
    ...
    def build_tree(self, node, depth, maximizing_player):
        ...
        best_score = float('-inf') if maximizing_player else 0
        for move in legal_moves:
            game_current.apply_move(move)
            child_node = ExpectimaxNode(game_current.get_state().copy())
            ...
            if maximizing_player:
                ...
                best_score += 1 / len(game_current.get_legal_moves()) * child_node.move
                # if child_node.score < best_score:
                #     best_score = child_node.score
    ...
    def get_best_move(self, node: ExpectimaxNode, is_maximizing_player):
